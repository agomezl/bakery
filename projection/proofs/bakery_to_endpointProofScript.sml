  ...
  >- (MAP_EVERY Q.EXISTS_TAC [`s`,`cut_sel_upto p c'`]
     ...
     \\ `project_ok q c''`
        by(imp_res_tac compile_network_ok_project_ok
           ...
           \\ Cases_on `b` \\ fs[])
      \\ first_assum drule
(*      \\ qmatch_goalsub_abbrev_tac `project_ok q iffy_c`*)
      \\ strip_tac
(*     \\ `project_ok q (IfThen v p (Sel p b q c'') (c2f q))` by fs[]
     \\ `project_ok q (c2f q)`
        by(fs[project_def] \\ rfs[split_sel_def]
           \\ rpt(PURE_FULL_CASE_TAC \\ fs[])
           \\ imp_res_tac split_sel_project_ok \\ fs[])*)
...
(*     \\ `b = T /\ ?c2'. split_sel q p (c2f q) = SOME (F,c2')` 
        by(fs[project_def,cut_ext_choice_upto_presel_def,preSel_def,
               split_sel_def,compile_network_gen_def,preSel_to_queue_def]
           >> rfs[] >> rpt(PURE_FULL_CASE_TAC >> fs[] >> rveq))*)
